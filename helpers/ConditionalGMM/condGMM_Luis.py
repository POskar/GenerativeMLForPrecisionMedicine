    ...
    def rvs(self, x2, size = 1, random_state = None, component_labels = False):
        ...
        for i in inds:
            ...
            if n == 0: #Skip if no draws
                continue
            rvs_i_temp = dists[i].rvs(x2 = x2, size = n)
            print('shape of temporary obj:', rvs_i_temp.shape)
            rvs_i = np.atleast_2d(rvs_i_temp)
            
            #QUICK FIX
            if len(rvs_i_temp.shape)==1:
                rvs_i = np.reshape(rvs_i_temp, (-1, 1))
            #
